#define S_FUNCTION_NAME PMU_Estimator_Phase_A /* Defines and Includes */
...
#define NPARAMS 10   //Total number of Parameters
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    mexPrintf("Phase a \n");
    mexPrintf(" _____Time_____ ");
    mexPrintf(" _____Amplitude_____ ");
    mexPrintf(" _____Phase Angle_____ ");
    mexPrintf(" _____Frequency_____ ");
...
int dft_r(double* in_ptr, complex* out_ptr , unsigned int out_len, int n_bins, double norm_factor ){
    ...
    for (k = 0 ; k < n_bins ; ++k)
    {
        ...
        out_ptr[k].r = 0;
        for (n=0 ; n<out_len ; ++n){
            out_ptr[k].r += (in_ptr[n] * cos(n * k * PI*2 / out_len))/norm_factor;
           
        } 
        // Imaginary part of X[k]
        out_ptr[k].i = 0;
        for (n=0 ; n<out_len ; ++n) {
            out_ptr[k].i -= (in_ptr[n] * sin(n * k * PI*2 / out_len))/norm_factor;
        }
    }
